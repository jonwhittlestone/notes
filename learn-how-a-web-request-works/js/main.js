...
(function (global) {
  ...
  function measureLayout() {
    ...
    /* The dock is fixed to the whole window, so without this its right-hand
       controls slide underneath the inspector on a desktop screen. */
    setVar('--panel-w', Math.round(mobile ? 0 : layout.panel) + 'px');
  }
  ...
  /* The default view rides with the trolley rather than showing the whole
     world, because the tour is about what the trolley is doing. Zooming out is
     one click, or a double-click on the map. */
  function defaultScale() { return viewW <= 900 ? 0.8 : 1.1; }

  /* (GW + GH) on both axes is the correct projected bounding box, not a
     shortcut: the ground plate is a diamond whose screen width is (GW + GH)·TW
     and whose screen height is (GW + GH)·TH. */
  function fitWorld() {
  ...
  /* Aim slightly along the direction of travel, so you see where the trolley is
     heading rather than where it has been. Small enough that a stopped trolley
  ...
  resize();
  measureLayout();
  UI.run();                 /* puts a trolley on the road, so it can be framed */
